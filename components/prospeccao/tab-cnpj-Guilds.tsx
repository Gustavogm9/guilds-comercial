...
export default function TabCnpj({ onEmpresaEnriquecida }: Props) {
  ...
  async function enriquecerSite() {
    ...
    try {
      // Envia CNPJ junto pra atualizar empresa persistida com dados web (Firecrawl)
      const cnpjLimpo = cnpj.replace(/\D/g, "");
      const res = await fetch("/api/prospeccao/enriquecer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: empresa.site, cnpj: cnpjLimpo.length === 14 ? cnpjLimpo : undefined }),
      });
      const data = await res.json();
