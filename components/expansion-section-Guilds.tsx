      ...
      {semDados ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KpiCard
            ...
            value={fmt(
              resumo
                ? resumo.pipeline_aberto * (resumo.taxa_conversao_pct != null ? resumo.taxa_conversao_pct / 100 : 0.3)
                : 0,
            )}
            sub={
              resumo && resumo.taxa_conversao_pct != null
                ? `${resumo.ativas ?? 0} oportunidades · ${resumo.taxa_conversao_pct}% conv.`
                : `${resumo?.ativas ?? 0} ops · 30% estimado (sem histórico)`
            }
