...
import {
  Sparkles, X, Loader2, ArrowRight, Plus, Trophy, CheckCircle2, MessageCircle,
} from "lucide-react";
import {
  responderPedidoIndicacao,
  obterOuCriarPedidoIndicacaoPosFechamento,
  type NovaIndicacaoInput,
} from "@/app/(app)/growth/indicacoes/actions";
import ScriptPedidoModal from "@/components/script-pedido-modal";
...
export default function FechamentoCelebrationModal({
  ...
  // Garante pedido_id: usa o que veio via prop OU cria/recupera sob demanda.
  async function resolverPedidoId(): Promise<number> {
    if (pedidoId) return pedidoId;
    if (leadId == null) throw new Error("Lead inválido.");
    return await obterOuCriarPedidoIndicacaoPosFechamento(leadId);
  }
  ...
  function handleSalvar() {
    ...
    startTransition(async () => {
      try {
        const pid = await resolverPedidoId();
  ...
  function handleSemIndicacoes() {
    startTransition(async () => {
      try {
        const pid = await resolverPedidoId().catch(() => null);
        if (pid) {
          await responderPedidoIndicacao({ pedido_id: pid, status: "negado" });
        }
        onClose();
      } catch (e) {
        setErro(e instanceof Error ? e.message : "Erro.");
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Sub-modal: script de pedido (lazy) */}
        {leadId != null && (
          <ScriptPedidoModal
            leadId={leadId}
            open={scriptOpen}
