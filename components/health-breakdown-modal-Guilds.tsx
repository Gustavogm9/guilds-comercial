...
export default function HealthBreakdownModal({
  ...
  useEffect(() => {
    trackFlywheelEvent("health_modal_aberto", { lead_id: leadId }).catch(() => {});
    const sb = createClient();
    let cancelado = false;
    (async () => {
      try {
        const res = await sb.rpc("health_breakdown_completo", { _lead_id: leadId });
        if (cancelado) return;
        if (res.error) throw res.error;
        const data = (res.data ?? null) as
          | { breakdown: HealthBreakdown | null; tendencia: HealthTendencia | null; nps_historico: NpsHistoricoLead | null }
          | null;
        if (!data?.breakdown) throw new Error("Lead sem health score calculado.");
        setBreakdown(data.breakdown);
        setTendencia(data.tendencia ?? null);
        setNpsHistorico(data.nps_historico ?? null);
        setLoading(false);
      } catch (e: unknown) {
        if (cancelado) return;
          ...
          {!loading && breakdown && (
            <>
              ...
              <div className="text-center mb-5">
                ...
                {tendencia && tendencia.tendencia_30d === "novo" && (
                  <div className="inline-flex items-center gap-1 mt-2 text-[11px] text-muted-foreground">
                    <span className="uppercase tracking-[0.1em] font-semibold">tendência</span>
                    <span>·</span>
                    <span>sem histórico ainda · 30d de coleta acumulando</span>
                  </div>
                )}
              </div>
