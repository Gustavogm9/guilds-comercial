...
const VARIACOES: { key: Variacao; label: string; desc: string; tone: string; border: string }[] = [
  { key: "conservadora", label: "Conservadora", desc: "Escopo mínimo, menor investimento, baixo risco.", tone: "text-slate-700", border: "border-slate-200 bg-slate-50/50" },
  { key: "recomendada", label: "Recomendada", desc: "Melhor custo-benefício, escopo equilibrado.", tone: "text-indigo-700", border: "border-indigo-200 bg-indigo-50/50 ring-2 ring-indigo-400 ring-offset-2" },
  { key: "premium", label: "Premium", desc: "Escopo completo, máximo valor entregue.", tone: "text-emerald-700", border: "border-emerald-200 bg-emerald-50/50" },
];
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {VARIACOES.map(v => (
          <div key={v.key} className={`card p-5 ${v.border} transition`}>
            ...
            {propostas[v.key] ? (
              <div className="text-xs text-slate-700 whitespace-pre-wrap leading-relaxed border-t border-slate-100 pt-3 mb-3 max-h-96 overflow-y-auto">
                {propostas[v.key]}
              </div>
            ) : gerando === v.key ? (
              <div className="flex items-center gap-2 py-8 justify-center text-indigo-500 text-xs">
