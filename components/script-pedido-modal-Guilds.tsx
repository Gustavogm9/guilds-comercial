...
export default function ScriptPedidoModal({
  ...
  useEffect(() => {
    if (!open) return;
    trackFlywheelEvent("script_pedido_aberto", { lead_id: leadId }).catch(() => {});
    setErro(null);
    setScript(null);
    let cancelado = false;
  ...
  async function copy(text: string, which: "curto" | "longo") {
    try {
      ...
      trackFlywheelEvent("script_pedido_copiado", { lead_id: leadId, versao: which }).catch(() => {});
      setCopiado(which);
