...
export type PushEvento =
  ...
  | "lead_reabriu"
  // Flywheel (lado direito do funil)
...
const EVENTO_LABEL: Record<PushEvento, { label: string; descricao: string; grupo: "pipeline" | "flywheel" }> = {
  cadencia_vencendo: {
  ...
  nps_detrator_recebido: {
    label: "NPS detrator",
    descricao: "Cliente respondeu NPS com nota ≤6. Ação rápida hoje pode salvar.",
    grupo: "flywheel",
  },
  indicacao_via_portal: {
    label: "Indicação via portal",
    descricao: "Embaixador acabou de submeter uma indicação nova pelo portal público.",
    grupo: "flywheel",
  },
  health_risco_critico: {
    label: "Cliente em risco crítico",
    descricao: "Health score caiu para categoria 'em risco'. Detecta churn antes que aconteça.",
    grupo: "flywheel",
  },
  renovacao_iminente: {
    label: "Renovação iminente (≤7d)",
    descricao: "Contrato vence em até 7 dias. Confirmar continuidade.",
    grupo: "flywheel",
  },
  expansao_atrasada: {
    label: "Expansão atrasada",
    descricao: "Oportunidade de upsell com próxima ação vencida. Fechar ou marcar como perdida.",
    grupo: "flywheel",
  },
};
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        ...
        <fieldset className="space-y-2">
          ...
          {TODOS_EVENTOS.filter((ev) => EVENTO_LABEL[ev].grupo === "flywheel").map((ev) => (
            <label
              key={ev}
              className={`flex items-start gap-3 p-3 rounded-lg border border-border cursor-pointer hover:bg-muted/30 ${ativo ? "" : "opacity-50"}`}
            >
