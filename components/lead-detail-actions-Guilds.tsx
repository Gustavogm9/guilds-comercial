...
import { MessageSquareQuote, Play, AlertCircle, CheckCircle2, X, Loader2, Stethoscope, Sparkles } from "lucide-react";
...
import AiSdrModal from "@/components/ai-sdr-modal";
...
export default function LeadDetailActions({
  ...
  const [aiSdrOpen, setAiSdrOpen] = useState(false);
  ...
  function handleChangeEtapa(novaEtapa: CrmStage) {
    ...
    start(async () => {
      try {
        const res = await moverEtapa(lead.id, novaEtapa);
        setFeedback({
          tipo: "sucesso",
        ...
        // moverEtapa retorna pedido_indicacao_id criado pelo trigger — sem race condition.
        if (novaEtapa === "Fechado" && lead.crm_stage !== "Fechado") {
          setPedidoIndicacaoId(res?.pedido_indicacao_id ?? null);
  ...
  return (
    <>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button
          onClick={() => setAiSdrOpen(true)}
          className="btn-ghost text-xs font-medium text-primary hover:bg-primary/10"
          type="button"
          title="Gera mensagem personalizada via IA"
        >
        ...
        <AiSdrModal open={aiSdrOpen} onClose={() => setAiSdrOpen(false)} leadId={lead.id} leadEmpresa={lead.empresa} />
        <MotivoSaidaModal
