...
import FechamentoCelebrationModal from "./fechamento-celebration-modal";
 ...
 */
export default function KanbanBoard({ leads, currency = "BRL" }: { leads: LeadEnriched[]; currency?: string }) {
  ...
  const [celebracao, setCelebracao] = useState<{ leadId: number; pedidoId: number | null; empresa: string | null; nome: string | null } | null>(null);
  ...
  function confirmarFechar() {
    ...
    start(async () => {
      try {
        // moverEtapa agora retorna o pedido_indicacao_id criado pelo trigger SQL
        // — eliminamos a race condition do setTimeout(200ms) anterior.
        const res = await moverEtapa(lead.id, "Fechado");
        setFeedback({
          tipo: "sucesso",
        ...
        setCelebracao({
          leadId: lead.id,
          pedidoId: res?.pedido_indicacao_id ?? null,
  ...
  return (
    <DndContext id="kanban-board" sensors={sensors} onDragStart={onStart} onDragEnd={onEnd}>
      ...
      <DragOverlay>
        {active && <Card lead={active} dragging t={t} currency={currency} />}
      </DragOverlay>
      ...
      {/* Modal celebração ao fechar (dispara após drag→Fechado) */}
      <FechamentoCelebrationModal
        open={!!celebracao}
        pedidoId={celebracao?.pedidoId ?? null}
        leadId={celebracao?.leadId}
        leadEmpresa={celebracao?.empresa ?? null}
        leadNome={celebracao?.nome ?? null}
...
function Column({ stage, stageLabel, ponderadoLabel, vazioLabel, leads, t, currency }: {
  stage: CrmStage;
  stageLabel: string;
      ...
      <div className="px-3 py-2 border-b border-border/60 dark:border-white/[0.06]">
        ...
        {totalValor > 0 && (
          <div className="text-[10px] text-muted-foreground mt-0.5 tabular-nums">
            {totalValor.toLocaleString("pt-BR", { style: "currency", currency, maximumFractionDigits: 0 })} {ponderadoLabel}
          </div>
        )}
      </div>
      <div className="flex-1 p-2 space-y-2 min-h-[200px] max-h-[calc(100vh-220px)] overflow-y-auto">
        {leads.map(l => <Card key={l.id} lead={l} t={t} currency={currency} />)}
        {leads.length === 0 && (
...
function Card({ lead, dragging = false, t, currency = "BRL" }: {
  lead: LeadEnriched;
  dragging?: boolean;
  t: (k: string) => string;
  currency?: string;
        ...
        <div className="flex-1 min-w-0">
          ...
          >
            <div className="flex items-start justify-between gap-1">
              ...
              {lead.score_total != null && <LeadScoreBadge score={lead.score_total} size="xs" />}
              {lead.prioridade === "A" && (
              ...
              <div className="flex flex-col items-end gap-0.5 text-right">
                {lead.valor_potencial > 0 && (
                  <span className="text-foreground/80 font-medium leading-none">
                    {lead.valor_potencial.toLocaleString("pt-BR", { style: "currency", currency, maximumFractionDigits: 0 })}
                  </span>
                )}
                {(lead.valor_setup > 0 || lead.valor_mensal > 0) && (
                  <span className="text-[9px] text-muted-foreground/80 leading-none">
                    {[
                      lead.valor_setup > 0 && `Setup: ${lead.valor_setup.toLocaleString("pt-BR", { style: "currency", currency, maximumFractionDigits: 0 })}`,
                      lead.valor_mensal > 0 && `MRR: ${lead.valor_mensal.toLocaleString("pt-BR", { style: "currency", currency, maximumFractionDigits: 0 })}`
                    ].filter(Boolean).join(" | ")}
                  </span>
                )}
              </div>
