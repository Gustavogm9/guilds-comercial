...
import { Telescope, Building2, FileText, ArrowRight, AlertTriangle, Star, Target } from "lucide-react";
...
export default async function ProspeccaoPage({ searchParams }: Props) {
  ...
  // Counts pros quick-access cards
  const [{ count: totalEmpresasLocal }, { count: alertasNovos }, { count: meusFavoritos }] = await Promise.all([
    supabase.from("prospeccao_empresa").select("id", { count: "exact", head: true }),
    supabase.from("v_prospeccao_alertas_org").select("id", { count: "exact", head: true })
      .eq("organizacao_id", orgId).eq("visto", false),
    supabase.from("prospeccao_empresa_bookmark").select("id", { count: "exact", head: true })
      .eq("profile_id", me.id).eq("organizacao_id", orgId),
  ]);
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      {/* Quick-access pra ferramentas paralelas */}
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
        <Link
          href="/vendas/prospeccao/base-de-empresas"
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-foreground text-sm inline-flex items-center gap-1">
                Base de empresas
                <ArrowRight className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
              <p className="text-xs text-muted-foreground mt-0.5">
                <strong className="text-foreground tabular-nums">{(totalEmpresasLocal ?? 0).toLocaleString("pt-BR")}</strong> empresas · filtros + export
              </p>
            </div>
          </div>
        </Link>
        ...
        <Link
          href="/vendas/prospeccao/favoritos"
          className="card p-4 hover:border-primary/40 transition-colors group"
        >
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-warning-500/10 text-warning-500 grid place-items-center shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {(meusFavoritos ?? 0) > 0
                  ? <><strong className="text-foreground tabular-nums">{meusFavoritos}</strong> empresa(s) salva(s)</>
                  : "Marque ⭐ pra acompanhar"
                }
              </p>
            </div>
          </div>
        ...
        <Link
          href="/vendas/prospeccao/icp-fit"
          className="card p-4 hover:border-primary/40 transition-colors group"
        >
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-primary/10 text-primary grid place-items-center shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Ranking por similaridade c/ clientes fechados (embeddings)
              </p>
            </div>
          </div>
        </Link>
        ...
        <Link
          href="/vendas/prospeccao/alertas"
          className={`card p-4 hover:border-primary/40 transition-colors group ${(alertasNovos ?? 0) > 0 ? "border-warning-500/30 bg-warning-500/[0.02]" : ""}`}
        >
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-warning-500/10 text-warning-500 grid place-items-center shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {(alertasNovos ?? 0) > 0
                  ? <><strong className="text-warning-500 tabular-nums">{alertasNovos}</strong> novo(s) hoje</>
                  : "Cron diário 04 UTC"
                }
              </p>
            </div>
        ...
        {isGestor && (
          <Link
            href="/vendas/prospeccao/bulk-import"
            className="card p-4 hover:border-primary/40 transition-colors group"
          >
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-lg bg-primary/10 text-primary grid place-items-center shrink-0">
