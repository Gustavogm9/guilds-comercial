...
import LigacaoTranscricaoPanel from "@/components/ligacao-transcricao-panel";
...
export default async function LeadDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(lead as any).score_total != null && (
                <LeadScoreBadge score={(lead as any).score_total} size="md" />
              )}
              {lead.is_demo && (
      ...
      {/* Análise de chamadas IA */}
      <section className="mt-6">
        <LigacaoTranscricaoPanel leadId={lead.id} />
      </section>

      {/* Campos customizados (se a org tiver definido) */}
      <section className="mt-6">
        <CustomFieldsPanel
          entidade="lead"
          entidadeId={lead.id}
          valoresIniciais={((lead as any).custom_fields ?? {}) as Record<string, any>}
        />
      </section>
