...
export default async function VendedorPage(
  ...
  if (role !== "gestor" && params.id !== me.id) redirect(`/gestao/vendedor/${me.id}`);
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          {isGestor && (
            <Link href="/gestao/time" className="btn-ghost text-muted-foreground hover:text-primary">
        ...
        <form className="flex items-center gap-1 text-xs">
          {(["7d","30d","all"] as const).map(p => (
            <Link key={p} href={`/gestao/vendedor/${params.id}?periodo=${p}`}
