...
import {
  ...
  criarTemplateOnboarding, adicionarItemTemplate, removerItemTemplate, atualizarItemTemplate,
  clonarTemplateComoDraft, publicarTemplateDraft, descartarTemplateDraft,
} from "./actions";
import {
...
function isTab(v: unknown): v is Tab {
  return typeof v === "string" && (TABS_VALIDAS as readonly string[]).includes(v);
}
...
export default function PosVendaClient({
  ...
  // Tab vem da URL (?tab=...) com fallback "onboarding"
  // Gestor-only "templates" cai pra onboarding se não-gestor
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div role="tablist" className="flex gap-1 border-b border-border mb-4 overflow-x-auto">
        ...
        <TabBtn v="saude" cur={tab} set={setTab}
          ...
          label={t("pos_venda.tab_saude").replace("{{n}}", healthResumo && healthResumo.em_risco > 0 ? ` (${healthResumo.em_risco})` : "")} />
        <TabBtn v="expansoes" cur={tab} set={setTab}
          icon={<Rocket className="w-3.5 h-3.5" />}
          label={t("pos_venda.tab_expansoes").replace("{{n}}", expansoesResumo && expansoesResumo.ativas > 0 ? ` (${expansoesResumo.ativas})` : "")} />
        <TabBtn v="renovacoes" cur={tab} set={setTab}
          icon={<RotateCw className="w-3.5 h-3.5" />}
          label={t("pos_venda.tab_renovacoes").replace("{{n}}", renovacoesLeads.filter(r => !r.data_renovacao).length > 0 ? ` (${renovacoesLeads.filter(r => !r.data_renovacao).length})` : "")} />
        {isGestor && (
...
function TemplateCard({ tpl, itens, t, onSucesso, onErro }: {
  ...
  function handleToggleObrigatorio(item_id: number, novo: boolean) {
    startTransition(async () => {
      try {
        await atualizarItemTemplate({ item_id, obrigatorio: novo });
        onSucesso(novo ? "Item marcado como obrigatório." : "Item agora é opcional.");
      } catch (e) { onErro(e); }
    });
  }

  // Inline confirm dialog state (substitui confirm() nativo)
  ...
  function handleClonarDraft() {
    setConfirmar({
      titulo: `Criar rascunho v${(tpl.versao ?? 1) + 1}?`,
      mensagem: "A versão atual continua ativa até você publicar o rascunho.",
      textoConfirmar: "Criar rascunho",
      tom: "info",
      onConfirm: () => {
        setConfirmar(null);
        startTransition(async () => {
          try {
            await clonarTemplateComoDraft(tpl.id);
            onSucesso("Rascunho criado. Edite os itens e clique em Publicar quando estiver pronto.");
          } catch (e) { onErro(e); }
        });
      },
    });
  ...
  function handlePublicarDraft() {
    setConfirmar({
      titulo: "Publicar este rascunho?",
      mensagem: "A versão anterior será arquivada e novos leads usarão a nova versão.",
      ...
      onConfirm: () => {
        ...
        startTransition(async () => {
          try {
            await publicarTemplateDraft(tpl.id);
            onSucesso("Rascunho publicado. Versão anterior arquivada.");
          } catch (e) { onErro(e); }
        });
      },
    });
  ...
  function handleDescartarDraft() {
    setConfirmar({
      ...
      mensagem: "Todas as edições serão perdidas. Não dá pra desfazer.",
      ...
      onConfirm: () => {
        ...
        startTransition(async () => {
          try {
            await descartarTemplateDraft(tpl.id);
            onSucesso("Rascunho descartado.");
          } catch (e) { onErro(e); }
        });
      },
    });
  ...
  const isArquivado = tpl.status === "arquivado";
  const versao = tpl.versao ?? 1;

  return (
    <div className={`card p-4 ${isDraft ? "border-warning-500/40 bg-warning-500/[0.02]" : ""} ${isArquivado ? "opacity-60" : ""}`}>
      <div className="flex items-start justify-between mb-3 gap-2 flex-wrap">
        <div className="min-w-0 flex-1">
          <div className="font-semibold text-sm flex items-center gap-2 flex-wrap">
            {tpl.nome}
            <span className="text-[10px] uppercase tracking-[0.12em] font-semibold text-muted-foreground bg-secondary px-1.5 py-0.5 rounded border border-border">
              v{versao}
            </span>
            {tpl.default_template && !isArquivado && (
            ...
            {isDraft && (
              <span className="text-[10px] uppercase tracking-[0.12em] font-semibold bg-warning-500/15 text-warning-500 px-1.5 py-0.5 rounded border border-warning-500/30">
                rascunho
              </span>
            )}
            {isArquivado && (
              <span className="text-[10px] uppercase tracking-[0.12em] font-semibold bg-muted text-muted-foreground px-1.5 py-0.5 rounded border border-border">
                arquivado
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {isDraft ? (
            <>
              <button onClick={handlePublicarDraft} disabled={pending} className="btn-primary text-xs">
                Publicar rascunho
              </button>
              <button onClick={handleDescartarDraft} disabled={pending} className="btn-ghost text-xs text-muted-foreground hover:text-destructive">
                Descartar
              </button>
            </>
          ) : !isArquivado ? (
            <button
              onClick={handleClonarDraft}
              disabled={pending}
              className="btn-ghost text-xs text-primary"
              title="Cria uma nova versão como rascunho. A versão atual continua ativa até você publicar."
            >
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    type="button"
                    onClick={() => handleToggleObrigatorio(it.id, !it.obrigatorio)}
                    disabled={pending}
                    className={`text-[10px] uppercase tracking-[0.1em] px-1.5 py-0.5 rounded border transition-colors ${
                      it.obrigatorio
                        ? "text-warning-500 border-warning-500/30 bg-warning-500/10 hover:bg-warning-500/15"
                        : "text-muted-foreground border-border hover:bg-secondary/50"
                    }`}
                    title={it.obrigatorio ? "Clique para tornar opcional" : "Clique para tornar obrigatório"}
                    aria-pressed={it.obrigatorio}
                  >
...
function ExpansoesTab({
  ...
  function handleRemover(id: number) {
    setConfirmRemover(id);
  }

  function executarRemover() {
    const id = confirmRemover;
    if (!id) return;
  ...
  return (
    <>
                  ...
                  <tr key={e.id} className={`hover:bg-secondary/60 dark:hover:bg-white/[0.04] ${atrasada ? "bg-warning-500/[0.03]" : ""}`}>
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        ...
                        title="Template de email/WhatsApp/call"
                      >
                        Proposta
                      </button>
                      <button
      ...
      <ConfirmDialog
        ...
        titulo="Remover esta expansão?"
        mensagem="Só expansões ativas podem ser removidas. Use 'Perdida' se já avançou em conversa."
        textoConfirmar="Remover"
        tom="danger"
        pending={pending}
        onConfirm={executarRemover}
