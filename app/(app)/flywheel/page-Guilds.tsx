...
export default async function FlywheelPage() {
  ...
  const [
    ...
    // Forecast aquisição: agrega todos os vendedores (v_forecast_mes vem por responsavel_id)
    supabase.from("v_forecast_mes").select("forecast_best, forecast_likely, forecast_worst, leads_altos, leads_ativos")
      .eq("organizacao_id", orgId),
  ]);

  // Agrega forecast (a view retorna 1 linha por responsavel_id)
  const forecastAcc = (forecastAquisicaoRes.data ?? []).reduce(
    (acc, r: any) => ({
      best: acc.best + Number(r.forecast_best ?? 0),
      likely: acc.likely + Number(r.forecast_likely ?? 0),
      worst: acc.worst + Number(r.forecast_worst ?? 0),
      leads_altos: acc.leads_altos + Number(r.leads_altos ?? 0),
      leads_ativos: acc.leads_ativos + Number(r.leads_ativos ?? 0),
    }),
    { best: 0, likely: 0, worst: 0, leads_altos: 0, leads_ativos: 0 },
  );
  ...
  return (
    <FlywheelDashboard
      ...
      forecastAquisicao={{
        best: forecastAcc.best,
        likely: forecastAcc.likely,
        worst: forecastAcc.worst,
      }}
    />
  );
}
