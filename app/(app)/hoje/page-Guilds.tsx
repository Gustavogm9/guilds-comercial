...
import HojeRealtimeRefresher from "@/components/hoje-realtime-refresher";
...
export default async function HojePage(props: { searchParams: Promise<{ todos?: string; welcome?: string }> }) {
  ...
  // Issue 13: lê moeda + timezone da org (default BRL/São Paulo)
  const { data: orgRow } = await supabase
    .from("organizacoes")
    .select("moeda_padrao, timezone")
    .eq("id", orgId)
    .maybeSingle();
  const currency = ((orgRow as any)?.moeda_padrao as string) || "BRL";
  const orgTimezone = ((orgRow as any)?.timezone as string) || "America/Sao_Paulo";
  ...
  // hoje_str no fuso da org (não UTC) — evita marcar lead como "parado" 3h cedo no BR
  const hoje_str = new Intl.DateTimeFormat("en-CA", {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      {/* Subscribe Supabase Realtime: mudanças em leads/pedidos/nps/expansoes/onboardings
          disparam router.refresh() (debounced 800ms) — atualiza alertas sem F5 */}
      <HojeRealtimeRefresher orgId={orgId} />
      ...
      <ActivationChecklist role={(role ?? "sdr") as "gestor" | "comercial" | "sdr"} marcos={marcos} userId={me.id} />
