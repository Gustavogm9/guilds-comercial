...
export async function moverEtapa(
...
): Promise<{ pedido_indicacao_id: number | null }> {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const orgId = await requireOrg();
  ...
  // Se foi pra Fechado, o trigger trg_pedido_apos_fechamento já criou um
  // pedido_indicacao 'pos_fechamento'. Lemos o id agora pra devolver pro
  // client e eliminar a race condition do setTimeout(200ms) que existia antes.
  let pedidoIndicacaoId: number | null = null;
  if (novaEtapa === "Fechado") {
    const { data: pedido } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    pedidoIndicacaoId = (pedido?.id as number | undefined) ?? null;
  }
  ...
  return { pedido_indicacao_id: pedidoIndicacaoId };
}
