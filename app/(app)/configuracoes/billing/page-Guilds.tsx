      ...
      <header className="mb-6 flex justify-end">
        <div className="rounded-lg border border-border bg-card px-4 py-3 min-w-[240px]">
          ...
          <div className="mt-1 flex items-center gap-2 font-semibold">
            ...
            {(() => {
              const st = currentOrg?.billing_status;
              if (st === "trialing") return "Trial";
              if (st === "active") return "Ativo";
              if (st === "past_due") return "Em atraso";
              if (st === "canceled") return "Cancelado";
              if (!st) return "Sem assinatura";
              return st;
            })()}
          </div>
          {trial.isTrial && (
      ...
      <section className="mt-6 grid md:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <p className="text-sm text-muted-foreground">
            Cobrança mensal via Stripe. Você pode trocar de plano a qualquer momento — pro upgrade
            cobramos só a diferença proporcional; pro downgrade aplica no próximo ciclo. Cancelamento
            sem multa.
          </p>
        </div>
        <div className="card p-5">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            Acompanhe o consumo no card acima e configure webhooks pra ser avisado de eventos.
          </p>
