...
// QR code só carrega quando user expande <details> — economiza bundle inicial
const QRCodeSVG = dynamic(
  () => import("qrcode.react").then((mod) => ({ default: mod.QRCodeSVG })),
  { ssr: false, loading: () => <div className="w-[200px] h-[200px] bg-muted/30 rounded mx-auto" /> }
);
...
export default function IndicarClient({
  ...
  // URL do portal — populada no mount (window não existe em SSR)
  const [portalUrl, setPortalUrl] = useState<string | null>(null);
  useEffect(() => { setPortalUrl(window.location.href); }, []);
  ...
  if (showSuccess) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-success-500/5 flex items-center justify-center p-4 relative">
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto p-4 md:p-8">
        ...
        {/* QR code pra compartilhar (Bloco F #15) — gerado client-side por qrcode.react */}
        {portalUrl && (
