...
export async function proxy(req: NextRequest) {
  ...
  const publicPrefixes = [
    ...
    "/api/convite",
    "/api/v1",
    "/api/webhooks",         // webhooks de providers externos (Brevo, etc.)
    "/auth/callback",
    "/aceitar-convite",      // página pública de signup via convite
    "/lp/",                  // landing pages públicas (capture forms)
  ];
  ...
  // ---------- User sem organização → /onboarding ----------
  // Quem confirma email mas não termina o wizard pode tentar acessar /hoje, /vendas,
  // etc. diretamente. Sem org, a página mostra estado "sem organização" sem ação.
  // Empurra pro wizard explicitamente. Evita loop: /onboarding e /trocar-senha
  // ficam fora do redirect.
  if (user && pathname !== "/onboarding" && pathname !== "/trocar-senha" && !isPublic) {
    const homeOrg = user.user_metadata?.home_organizacao_id;
    if (!homeOrg) {
      // Confere via DB se realmente não tem org (user_metadata pode estar atrasado)
